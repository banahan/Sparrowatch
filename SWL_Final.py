...
if folders == 0:
	print(co("[ERROR] USB DRIVE FAILED TO MOUNT CORRECTLY RESETTING NOW",	'blue', attrs = ['bold']))
	GPIO.output(Red_LED, False) 	
	GPIO.output(Green_LED, False) 
...
print(co("[CONFIG] Operate Start Time is:   {}"			. format(Operate_Start_Time)	,'blue', attrs=['reverse','bold']))
print(co("[CONFIG]      Notify Start Time is:   {}"		. format(Notify_Start_Time)		,'blue', attrs=['reverse','bold']))
print(co("[CONFIG]      Notify Stop Time is:    {}"		. format(Notify_Stop_Time)		,'blue', attrs=['reverse','bold']))
print(co("[CONFIG] Operate Stop Time is:    {}"			. format(Operate_Stop_Time)		,'blue', attrs=['reverse','bold']))
...
if Connection_Status == True:
	...
	for i in range(len(email)):
		print(co("[EMAIL] Sending Email to {}.". format(email[i]),'white', attrs=['bold']))
		...
		try: 
			...
			if Oppy_Time == True:   
				...
				if frame is None:
					...
					break
				if Check(Shutdown_GPIO):
					print(co("[SHUTDOWN] User Shutdown called", 'magenta', attrs = ['bold']))
					break
									...
									if Noty_User == True:
										...
										if Seconds_Since_Last_Email >= Email_Waiting_Time:                                           
											...
											if Connection_Status == True:
												...
												if len_email == 1:
													send_email(email, Frame_Location, Frame_Count, Person_countr, Device_ID)
			...
			if Oppy_Time == False:
				...
				#print(co("[DEBUG] Op Loop = OFF. Noty Loop = OFF. A = {}; B = {}; Time is {}". format(Oppy_Time, Noty_User, XX) ,'white', attrs=['reverse','bold']))
				time.sleep(1)
