	...
	def Capture_Frame(self, Folder, DISPLAY_DIMS=(1280,720)):
		...
		try:
			...
			if Start_Up_Frame is None:
				print(co("[ERROR] Class Baseline; Capture_Frame function -- Error Reading the frame, rebooting now", 'cyan', attrs = ['bold']))
				GPIO.output(Red_LED,	 False) 
				GPIO.output(Green_LED,	 False) 
			...
			print(co("[ERROR] Class Baseline; Capture_Frame function -- Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
			GPIO.output(Red_LED,	 False) 
			GPIO.output(Green_LED,	 False) 
			GPIO.output(Yellow_LED,	 True) 
			time.sleep(0.5)
...
class Emailling_Users():
	def __init__(self):
		print("THIS FUNCTION DOES NOT WORK -- WHY ARE YOU HERE??")
		pass
	...
	def Detect_Internet_Connection(self):
		...
		try:
			...
			print(co("[ERROR] Class Emailing_Users; Detect_Internet_Connection function -- Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
			conn.close()
			Status = False
			Error = error_1
		return Status, Error
	...
	def Send_Alert_Email(self, End_User_Email, Frame_Path, Frame_Count, Person_Countr, Device_ID):
		...
		try:
			...
			#print(co("[ERROR] Class Emailing_Users; Send_Alert_Email function -- Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
			pass
		return
...
class Event_DB():
	...
	def Create_Event(self, Table_name):
		...
		try:
			...
			print(co("[ERROR] Class Event_DB; Create_Event function -- OperationalError Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
			self.connection.rollback()
		except Exception as Gen_Error:
			print(co("[ERROR] Class Event_DB; Create_Event function -- Exception Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
			self.connection.rollback()
	...
	def Event_Logger(self, Event_Trigger_Type, Email_Details, FOIC, Frame_Location):
		try:
			...
			print(co("[ERROR] Class Event_DB; Event_Logger function -- Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
	...
	def Update_Event(self, TableName, Time_Of_Event, Event_Trigger_Type, CPU_Temperature, Email_Details, FOIC, Frame_Location):
		...
		try:
			...
			print(co("[ERROR] Class Event_DB; Update_Event function -- Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
			self.connection.rollback()
...
class Heart_DB():
	...
	def Create_Heart(self, Table_name):
		...
		try:
			...
			print(co("[ERROR] Class Heart_DB; Create_Heart function -- OperationalError Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
			self.connection.rollback()
		except Exception as Gen_Error:
			print(co("[ERROR] Class Heart_DB; Create_Heart function -- Exception Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
			self.connection.rollback()
	def Close(self):
		self.connection.close()
	def Heart_Logger(self):
		try:
			...
			print(co("[ERROR] Class Heart_DB; Heart_Logger function -- Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
			print(co("																							"))
	...
	def Update_Heart(self, TableName, Heartbeat_Time, CPU_Temperature, Message):
		...
		try:
			...
			print(co("[ERROR] Class Heart_DB; Update_Heart function -- Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
			self.connection.rollback()
...
def Check_Internet_Connection():
		...
		try:
			...
			print(co("[ERROR] Check_Internet_Connection function -- Error Code: {}". format(Err),'cyan',attrs=['reverse','bold']))
			conn.close()
			Status = False
		return Status, Err
		...
		for dir in d:
			...
			if (Now - Day_Cutoff_in_Seconds) > timestamp:
				try:
					...
					print(co("[ERROR] FileRemover function -- Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
					pass
...
def Send_Alert_Email(End_User_Email, Frame_Path, Frame_Count, Person_Countr, Device_ID):
	...
	try:
		...
		#print(co("[ERROR] Send_Alert_Email function -- Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
		pass
	return
...
def MotionDetect(Frame, avg):
	...
	while a == True:
		try:
			...
			print(co("[ERROR] MotionDetect function -- Error Code: {}". format(error),'cyan',attrs=['reverse','bold']))
			a = False
	return motion, avg

def Shutdown_User_Button(pin_to_check):
	pin_state = GPIO.input(pin_to_check)
	return pin_state
